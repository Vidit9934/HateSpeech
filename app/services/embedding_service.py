...
from qdrant_client.http.models import Distance, VectorParams, PointStruct, SearchRequest, Filter, FieldCondition, MatchValue
...
class EmbeddingService:
    ...
    def _init_qdrant(self) -> None:
        ...
        try:
            ...
            logger.info(f"Collection info - vectors count: {collection_info.vectors_count}, "
    ...
    def embed_text(self, text: str, use_cache: bool = True, store_in_qdrant: bool = False, metadata: Optional[Dict[str, Any]] = None) -> np.ndarray:
        """
        ...
        try:
            ...
            logger.debug(f"Embedded text in {processing_time:.3f}s (length: {len(text)})")
    ...
    def embed_batch(
        ...
        if texts_to_embed:
            try:
                ...
                for i, (idx, embedding) in enumerate(zip(indices_to_compute, new_embeddings)):
                    embeddings[idx] = embedding
                    
                    # Cache the result
                    if use_cache and i < len(cache_keys) and cache_keys[idx] is not None:
        ...
        try:
            ...
            for doc in documents:
                if text_field in doc and doc[text_field]:
                    ...
                    logger.warning(f"Document missing '{text_field}' field or empty content")
            ...
            if store_in_qdrant:
                logger.info(f"Documents stored in Qdrant collection: {self.collection_name}")
    ...
    def semantic_search(
        ...
        try:
            ...
            query_embedding = self.embed_text(query, use_cache=True, store_in_qdrant=False)
