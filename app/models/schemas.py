...
class ClassificationRequest(BaseModel):
    ...
    timestamp: Optional[datetime] = Field(
        ...
        description="When the content was posted"
    )
...
class PolicyDocument(BaseModel):
    ...
    title: str = Field(
        ...,
        description="Title or identifier of the policy document"
    )
    
    content: str = Field(
        ...,
        description="The relevant policy text content"
    )
    ...
    section: Optional[str] = Field(
        default=None,
        description="Specific section of the policy document"
    )
...
class PolicyReasoning(BaseModel):
    ...
    applicable_policies: List[PolicyDocument] = Field(
        default_factory=list,
        description="List of relevant policy documents"
    )
    
    policy_violations: List[str] = Field(
        default_factory=list,
        description="Specific policy violations identified"
    )
    
    legal_implications: Optional[str] = Field(
        default=None,
        description="Potential legal implications if applicable"
    )
...
class ActionRecommendation(BaseModel):
    ...
    escalation_needed: bool = Field(
        default=False,
        description="Whether human moderator escalation is needed"
    )
    
    follow_up_actions: List[str] = Field(
        default_factory=list,
        description="Additional follow-up actions to consider"
    )
    
    ban_duration: Optional[int] = Field(
        default=None,
        description="Duration in hours for temporary bans"
    )
...
class ErrorInfo(BaseModel):
    ...
    fallback_result: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Fallback result if recovery was successful"
    )
...
class ModerationResponse(BaseModel):
    ...
    platform: Platform = Field(
        ...,
        description="Platform where the content was found"
    )
    ...
    warnings: List[str] = Field(
        default_factory=list,
        description="Warnings or notices about the analysis"
    )
...
class SystemHealth(BaseModel):
    ...
    model_status: str = Field(
        default="unknown",
        description="Status of the ML models"
    )
...
class BatchRequest(BaseModel):
    ...
    platform: Optional[Platform] = Field(
        default=Platform.GENERAL,
        description="Platform for all texts in the batch"
    )
