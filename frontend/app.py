...
st.set_page_config(
    page_title="Hate Speech Detection Tool", page_icon="🛡️", layout="wide"
)

st.title("🛡️ Hate Speech Detection Tool")
st.markdown(
    "Enter text content to analyze for hate speech, toxicity, and policy violations."
)
    ...
    with st.spinner("Analyzing content..."):
        try:
            ...
            response = requests.post(
                f"{API_BASE_URL}/moderate", json={"text": user_input}
            )
            ...
            if response.status_code == 200:
                ...
                if result["success"]:
                    ...
                    for policy in result["retrieved_policies"]:
                        with st.expander(
                            f"📄 {policy['filename']} (Score: {policy['relevance_score']:.3f})"
                        ):
                    ...
                    st.markdown(
                        f"### {action_colors.get(action, '⚪')} {action.upper()}"
                    )

                else:
                    st.error(
                        f"Analysis failed: {result.get('error_message', 'Unknown error')}"
                    )
            else:
